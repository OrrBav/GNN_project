...
OUTPUT_PATH = f"./outputs/graphs_sqrt(n)_{THRESHOLD}/"
OUTPUT_PATH_NODE_LIST = f"./outputs/node_lists_{THRESHOLD}/"
RESULT_CSV_PATH = f"./outputs/graphs_sqrt(n)_{THRESHOLD}/results_sqrt(n)_{THRESHOLD}.csv"
GRAPHS_PATH = f"./graphs/sqrt(n)_{THRESHOLD}/"
...
def create_edges(og_df, threshold, data_type):
    ...
    sequences_df.to_csv(OUTPUT_PATH_NODE_LIST+f"node_list_{threshold}_{data_type}.csv", index=False)
...
def plot_spectral_embedding(embedding, color, graph_name):
    ...
    plot_filename = OUTPUT_PATH+f"{graph_name}_spectral2D.png"
...
def plot_spectral_comb(embeddings, colors, threshold):
    ...
    for embedding, color in zip(embeddings, colors):
        plt.scatter(embedding[:, 0], embedding[:, 1], c=color, label=f'Embedding with color {color}')
    plt.title(f'Spectral Embedding for {threshold} ({colors[0]}: H, {colors[1]}: OC)')
    plt.xlabel('Dimension 1')
    plt.ylabel('Dimension 2')
    plt.legend() 
    plot_filename = OUTPUT_PATH+f"{threshold}_spectral2D_combined.png"
    plt.savefig(plot_filename)
    plt.close()
    return plot_filename
...
def plot_spectral_embedding_3d(embedding, color, graph_name):
    ...
    ax.scatter(embedding[:, 0], embedding[:, 1], embedding[:, 2], c=color)
    ax.title(f'Spectral Embedding for {graph_name}')
    ax.set_xlabel('Dimension 1')
    ax.set_ylabel('Dimension 2')
    ax.set_zlabel('Dimension 3')
    plot_filename = OUTPUT_PATH+f"{graph_name}_spectral3D.png"
    plt.savefig(plot_filename)
    plt.close()
    return plot_filename
...
def draw_both_graphs(G1, G2, threshold):
    ...
    plot_filename = OUTPUT_PATH+f"{threshold}_default_combined.png"
...
def draw_plots(G, graph_name, color, threshold):
    ...
    plot_filename_default = OUTPUT_PATH+f"{graph_name}_default.png"
...
def load_graph_from_graphml(file_name):
    G = nx.read_graphml(file_name)
    print(f"Loaded graph from {file_name}")
    return G

if __name__ == "__main__":
    graphs = {
    "OC_10_graph": load_graph_from_graphml(GRAPHS_PATH + "OC_10_" + str(THRESHOLD)+".graphml"),
    "H_10_graph": load_graph_from_graphml(GRAPHS_PATH + "H_10_" + str(THRESHOLD)+".graphml"),
    "OC_8_graph": load_graph_from_graphml(GRAPHS_PATH + "OC_8_" + str(THRESHOLD)+".graphml"),
    "H_8_graph": load_graph_from_graphml(GRAPHS_PATH + "H_8_" + str(THRESHOLD)+".graphml"),
    "OC_4_graph": load_graph_from_graphml(GRAPHS_PATH + "OC_4_" + str(THRESHOLD)+".graphml"),
    "H_4_graph": load_graph_from_graphml(GRAPHS_PATH + "H_4_" + str(THRESHOLD)+".graphml"),
}
    # create the H and OC graphs
    results = []
    for group in ["10", "8", "4"]:
        start_time = time.time()
        OC_graph_key = f"OC_{group}_graph"
        H_graph_key = f"H_{group}_graph"
        
        OC_graph = graphs.get(OC_graph_key)
        H_graph = graphs.get(H_graph_key)

        # Analyze H graph
        if H_graph.number_of_edges() == 0:
            result_H, embedding_H = handle_empty_graph(f"H_{group}", int(group))
            print("H_{group} is empty")
        else:
            result_H, embedding_H  = analyze_graph(G=H_graph, threshold=int(group), graph_name=f"H_{group}")
        print(f"Finished with graph H")
        # Analyze OC graph
        if OC_graph.number_of_edges() == 0:
            result_OC, embedding_OC = handle_empty_graph(f"OC_{group}", int(group))
            print("OC_{group} is empty")
        else:
            result_OC, embedding_OC = analyze_graph(G=OC_graph, threshold=int(group), graph_name=f"OC_{group}")
        ...
        if H_graph.number_of_edges() > 0 and OC_graph.number_of_edges() > 0:
            combined_plot_filename = draw_both_graphs(H_graph, OC_graph, int(group))
        ...
        if embedding_H is not None and embedding_OC is not None:
            comb_spectral_filename = plot_spectral_comb([embedding_H, embedding_OC], ["blue", "red"], int(group))
        ...
        print(f"Successfully finished working on threshold {int(group)}")
        
        end_time = time.time()  
        duration = (end_time - start_time) / 60 
        print(f"Threshold {int(group)} processed in {duration:.2f} minutes")

    result_df = pd.DataFrame(results)
    result_df.to_csv(RESULT_CSV_PATH, index=False)
    print(f"CSV file has been successfully created at: {RESULT_CSV_PATH}")
